...
Implement Priority Scheduling Non-preemtive version
...
void sort(struct queue *q)
{
	...
	while(t->next!=NULL)
	{
		...
		while(t2)
		{
			if(t->pcb.priority>t2->pcb.priority)
			{
...
{	
	sort(q);
	struct node * t = q->front;
	printf("PID\tIn Time\tBurst Time\tWait Time\tTurn Around Time\tPriority\n");
	float avg_wait = 0;
	...
	{	
		...
		printf("%d\t%.2f\t%.2f\t\t%.2f\t\t%.2f\t\t\t%d\n",t->pcb.pid,t->pcb.in_time,t->pcb.burst,t->pcb.wait_time,t->pcb.turn_around_time,t->pcb.priority);
		t = t->next;
	}
